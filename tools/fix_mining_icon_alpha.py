#!/usr/bin/env python3
"""Remove flat grey/white backgrounds from mining inventory PNGs."""
...
def process(path: Path, tolerance: float = 52.0) -> bool:
    ...
    for y in range(h):
        for x in range(w):
            ...
            if color_dist(rgb, bg) <= tolerance:
                ...
                changed = True
                continue
            # common AI export: neutral light grey slab
            if r > 165 and g > 165 and b > 165 and max(r, g, b) - min(r, g, b) < 28:
                px[x, y] = (r, g, b, 0)
                changed = True
    if changed:
...
def main() -> int:
    ...
    for path in sorted(IMAGES_DIR.glob("*.png")):
        if not any(path.name.startswith(p) for p in PATTERNS):
            continue
        if process(path):
