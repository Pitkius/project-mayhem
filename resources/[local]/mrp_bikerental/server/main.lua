...
RegisterNetEvent('mrp_bikerental:server:rent', function(locationId, model)
    local src = source
    ...
    TriggerClientEvent('mrp_bikerental:client:spawnBike', src, locationId, bike.model, makePlate())
end)

RegisterNetEvent('mrp_bikerental:server:returnBike', function(locationId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    ...
    TriggerClientEvent('mrp_bikerental:client:returnedBike', src, refund)
end)
