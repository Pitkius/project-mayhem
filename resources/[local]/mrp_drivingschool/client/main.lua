...
local function createBlip()
    ...
    exports['mrp_fonts']:SetBlipName(blip, cfg.label or 'Vairavimo mokykla')
end
        ...
        options = {
            {
                ...
                action = function()
                    TriggerEvent('mrp_drivingschool:client:openMenu')
                end,
            },
        },
...
local function failPractical(reason)
    ...
    TriggerServerEvent('mrp_drivingschool:server:practicalResult', practicalState.category, false)
    cleanupPractical()
end
...
local function passPractical()
    ...
    TriggerServerEvent('mrp_drivingschool:server:practicalResult', practicalState.category, true)
    cleanupPractical()
end
...
local function updateCheckpointBlip()
    ...
    exports['mrp_fonts']:SetBlipName(blip, ('Egzaminas — taškas %s/%s'):format(idx, #Config.RouteCheckpoints))
...
local function showPracticalOfferMenu(category, score, total)
    ...
    exports['qb-menu']:openMenu({
        ...
        },
        {
            ...
            params = {
                event = 'mrp_drivingschool:client:beginPractical',
...
RegisterNetEvent('mrp_drivingschool:client:beginPractical', function(data)
    local category = type(data) == 'table' and data.category or nil
    if not category then return end

    QBCore.Functions.TriggerCallback('mrp_drivingschool:server:canStartPractical', function(ok, msg)
        if not ok then
...
RegisterNetEvent('mrp_drivingschool:client:startPractical', function(category)
    startPracticalExam(category)
end)

RegisterNetEvent('mrp_drivingschool:client:openMenu', function()
    QBCore.Functions.TriggerCallback('mrp_drivingschool:server:getLicenceStatus', function(status)
        status = status or {}
        ...
        for _, key in ipairs({ 'a', 'b', 'c' }) do
            ...
            if cat then
                ...
                menu[#menu + 1] = {
                    ...
                    params = {
                        event = 'mrp_drivingschool:client:confirmExam',
                        args = { category = key },
                    },
                }
            end
...
RegisterNetEvent('mrp_drivingschool:client:showLicense', function(info)
    QBCore.Functions.TriggerCallback('mrp_drivingschool:server:getLicenseCard', function(card)
        if not card then
...
RegisterNetEvent('mrp_drivingschool:client:confirmExam', function(data)
    local category = type(data) == 'table' and data.category or nil
    ...
    exports['qb-menu']:openMenu({
        { header = cat.label, isMenuHeader = true },
        {
            ...
            params = {
                event = 'mrp_drivingschool:client:beginTheory',
                args = { category = category },
            },
        },
        {
            header = 'Atgal',
            params = { event = 'mrp_drivingschool:client:openMenu' },
        },
    })
end)

RegisterNetEvent('mrp_drivingschool:client:beginTheory', function(data)
    local category = type(data) == 'table' and data.category or nil
    if not category then return end
    ...
    QBCore.Functions.TriggerCallback('mrp_drivingschool:server:startTheory', function(ok, payload, msg)
        if not ok then
        ...
        TriggerEvent('mrp_drivingschool:client:showTheoryQuestion')
    end, category)
end)

RegisterNetEvent('mrp_drivingschool:client:showTheoryQuestion', function()
    ...
    for ai, ans in ipairs(q.answers) do
        menu[#menu + 1] = {
            header = ans,
            params = {
                event = 'mrp_drivingschool:client:theoryPick',
                args = { chosen = ai },
            },
        }
    end
...
RegisterNetEvent('mrp_drivingschool:client:theoryPick', function(data)
    if not theoryState then return end
    local chosen = type(data) == 'table' and data.chosen or nil
    if not chosen then return end

    QBCore.Functions.TriggerCallback('mrp_drivingschool:server:submitTheoryAnswer', function(result)
