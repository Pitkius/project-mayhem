...
RegisterNetEvent('fivempro_siren:server:setTone', function(netId, tone)
    local src = source
...
RegisterNetEvent('fivempro_siren:server:setMuted', function(netId, muted)
    local src = source
    netId = tonumber(netId)
    muted = muted == true
    if not netId or not NetworkDoesNetworkIdExist(netId) then return end
...
RegisterNetEvent('fivempro_siren:server:setEmsEmergencyMode', function(mode)
    local src = source
...
RegisterNetEvent('fivempro_siren:server:setEmsEmergencyKit', function(equip)
    local src = source
...
RegisterNetEvent('fivempro_siren:server:clearEmsEmergencyOnExit', function(netId)
    local src = source
    netId = tonumber(netId)
    if not netId or not NetworkDoesNetworkIdExist(netId) then return end
    if not isEmsOnDuty(src) then return end
