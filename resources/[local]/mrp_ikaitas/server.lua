...
local function clearHostage(aggressor, victim, reason)
    if aggressor then
        ...
        TriggerClientEvent('mrp_ikaitas:client:stop', aggressor, reason or 'stop')
    end
    if victim then
        activeVictim[victim] = nil
        TriggerClientEvent('mrp_ikaitas:client:stop', victim, reason or 'stop')
    end
end
...
RegisterNetEvent('mrp_ikaitas:server:tryStart', function(targetId)
    local src = source
    ...
    TriggerClientEvent('mrp_ikaitas:client:beAggressor', src, targetId)
    TriggerClientEvent('mrp_ikaitas:client:beVictim', targetId, src)
    notify(src, 'Įkaitas paimtas. G — paleisti, H — nusauti.', 'success')
    notify(targetId, 'Tave laiko įkaitu. Nusigink.', 'error')
end)

RegisterNetEvent('mrp_ikaitas:server:release', function()
    local src = source
...
RegisterNetEvent('mrp_ikaitas:server:kill', function()
    local src = source
    local victim = activeAggressor[src]
    if not victim then return end
    TriggerClientEvent('mrp_ikaitas:client:executeVictim', victim)
    TriggerClientEvent('mrp_ikaitas:client:stop', src, 'kill')
    TriggerClientEvent('mrp_ikaitas:client:stop', victim, 'kill')
    activeAggressor[src] = nil
    activeVictim[victim] = nil
    notify(src, 'Įkaitas nusaustas.', 'error')
end)

RegisterNetEvent('mrp_ikaitas:server:abort', function()
    local src = source
