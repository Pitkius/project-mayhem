...
local function notifyStaffNewReport(report)
    ...
    for _, playerId in ipairs(QBCore.Functions.GetPlayers()) do
        if isStaff(playerId) then
            ...
            TriggerClientEvent('mrp_reports:client:staffAlert', playerId, report.id)
            TriggerClientEvent('mrp_reports:client:refreshAdmin', playerId)
        end
    end
end
...
local function setReportStatus(source, reportId, status)
    ...
    if target then
        ...
        TriggerClientEvent('mrp_reports:client:refreshPlayer', report.source)
    end
...
QBCore.Functions.CreateCallback('mrp_reports:server:submit', function(source, cb, payload)
...
QBCore.Functions.CreateCallback('mrp_reports:server:myReports', function(source, cb)
    cb(getReportsForPlayer(source))
end)

QBCore.Functions.CreateCallback('mrp_reports:server:adminReports', function(source, cb)
    if not isStaff(source) then cb(nil) return end
    cb(getReportsForStaff(source))
end)

QBCore.Functions.CreateCallback('mrp_reports:server:setStatus', function(source, cb, reportId, status)
    local ok, err = setReportStatus(source, reportId, status)
    cb({ ok = ok, error = err })
end)

QBCore.Functions.CreateCallback('mrp_reports:server:reply', function(source, cb, reportId, text)
...
QBCore.Commands.Add('report', 'Atidaryti pagalbos centrą / reportą', {}, false, function(source)
    TriggerClientEvent('mrp_reports:client:open', source, 'create')
end, 'user')

QBCore.Commands.Add('myreports', 'Mano reportai', {}, false, function(source)
    TriggerClientEvent('mrp_reports:client:open', source, 'mine')
end, 'user')
...
QBCore.Commands.Add('reports', 'Admin reportų panelė', {}, false, function(source)
    ...
    TriggerClientEvent('mrp_reports:client:open', source, 'admin')
end, 'mod')
...
AddEventHandler('mrp_reports:internal:logCompat', function() end)
