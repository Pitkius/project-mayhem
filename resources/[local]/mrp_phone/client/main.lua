...
local function fetchPhoneData(cb)
    QBCore.Functions.TriggerCallback('mrp_phone:server:getInitialData', function(data)
...
RegisterNetEvent('mrp_phone:client:refreshData', function()
    if phonePhase ~= 'open' then return end
    fetchPhoneData(function(data)
        sendUi('hydrate', data)
    end)
...
RegisterNetEvent('mrp_phone:client:newMessageNotify', function(fromNumber)
...
RegisterNetEvent('mrp_phone:client:incomingCall', function(call)
    if phonePhase ~= 'open' then
...
RegisterNetEvent('mrp_phone:client:callState', function(call)
    call = call or {}
...
RegisterNetEvent('mrp_phone:client:closePhone', function()
    closePhone()
end)
...
RegisterNUICallback('saveContact', function(data, cb)
    QBCore.Functions.TriggerCallback('mrp_phone:server:saveContact', function(res)
        cb(res or { ok = false })
    end, data or {})
end)

RegisterNUICallback('updateContact', function(data, cb)
    QBCore.Functions.TriggerCallback('mrp_phone:server:updateContact', function(res)
        cb(res or { ok = false })
    end, data or {})
end)

RegisterNUICallback('deleteContact', function(data, cb)
    QBCore.Functions.TriggerCallback('mrp_phone:server:deleteContact', function(res)
        cb(res or { ok = false })
    end, data or {})
end)

RegisterNUICallback('sendMessage', function(data, cb)
    QBCore.Functions.TriggerCallback('mrp_phone:server:sendMessage', function(res)
        cb(res or { ok = false })
    end, data or {})
end)

RegisterNUICallback('getConversation', function(data, cb)
    QBCore.Functions.TriggerCallback('mrp_phone:server:getConversation', function(res)
        cb(res or { ok = false, messages = {} })
    end, data or {})
end)

RegisterNUICallback('createAd', function(data, cb)
    QBCore.Functions.TriggerCallback('mrp_phone:server:createAd', function(res)
        cb(res or { ok = false })
    end, data or {})
end)

RegisterNUICallback('deleteAd', function(data, cb)
    QBCore.Functions.TriggerCallback('mrp_phone:server:deleteAd', function(res)
        cb(res or { ok = false })
    end, data or {})
end)

RegisterNUICallback('createPost', function(data, cb)
    QBCore.Functions.TriggerCallback('mrp_phone:server:createPost', function(res)
        cb(res or { ok = false })
    end, data or {})
end)

RegisterNUICallback('likePost', function(data, cb)
    QBCore.Functions.TriggerCallback('mrp_phone:server:likePost', function(res)
        cb(res or { ok = false })
    end, data or {})
end)

RegisterNUICallback('startCall', function(data, cb)
    TriggerServerEvent('mrp_phone:server:startCall', data or {})
    cb({ ok = true })
end)

RegisterNUICallback('respondCall', function(data, cb)
    TriggerServerEvent('mrp_phone:server:respondCall', data or {})
    cb({ ok = true })
end)

RegisterNUICallback('endCall', function(data, cb)
    TriggerServerEvent('mrp_phone:server:endCall', data or {})
    cb({ ok = true })
end)

RegisterNUICallback('emergencyCall', function(data, cb)
    local c = GetEntityCoords(PlayerPedId())
    TriggerServerEvent('mrp_phone:server:emergencyCall', data and data.service or '', {
        x = c.x,
        y = c.y,
        z = c.z,
    })
...
RegisterNUICallback('createAccount', function(data, cb)
    QBCore.Functions.TriggerCallback('mrp_phone:server:createAccount', function(res)
        cb(res or { ok = false })
    end, data or {})
end)

RegisterNUICallback('installApp', function(data, cb)
    QBCore.Functions.TriggerCallback('mrp_phone:server:installApp', function(res)
        cb(res or { ok = false })
    end, data or {})
end)
...
RegisterNUICallback('getPhoto', function(data, cb)
    QBCore.Functions.TriggerCallback('mrp_phone:server:getPhoto', function(res)
        cb(res or { ok = false })
    end, data or {})
end)

RegisterNUICallback('saveNotes', function(data, cb)
    QBCore.Functions.TriggerCallback('mrp_phone:server:saveNotes', function(res)
        cb(res or { ok = false })
    end, data or {})
end)

RegisterNUICallback('deleteNote', function(data, cb)
    QBCore.Functions.TriggerCallback('mrp_phone:server:deleteNote', function(res)
        cb(res or { ok = false })
    end, data or {})
end)

RegisterNUICallback('deleteOldNotes', function(data, cb)
    QBCore.Functions.TriggerCallback('mrp_phone:server:deleteOldNotes', function(res)
        cb(res or { ok = false })
    end, data or {})
end)
...
RegisterNUICallback('getAdProfileAvatar', function(data, cb)
    QBCore.Functions.TriggerCallback('mrp_phone:server:getAdProfileAvatar', function(res)
        cb(res or { ok = false })
    end, data or {})
end)
...
RegisterNUICallback('getWeatherForecast', function(data, cb)
    ...
    if GetResourceState('mrp_weather') ~= 'started' then
        cb({ ok = false, error = 'weather_module' })
        return
    end
    QBCore.Functions.TriggerCallback('mrp_weather:server:getForecast', function(res)
        QBCore.Functions.TriggerCallback('mrp_weather:server:getRegions', function(regRes)
            res = res or { ok = false }
...
RegisterNUICallback('getWeatherRegions', function(_, cb)
    ...
    QBCore.Functions.TriggerCallback('mrp_weather:server:getRegions', function(res)
        cb(res or { ok = false })
    end)
end)
...
RegisterNUICallback('getWeather', function(_, cb)
    if GetResourceState('mrp_weather') == 'started' then
        local ok, slot = pcall(function()
            return exports['mrp_weather']:getCurrentSlot()
        end)
        if ok and slot then
...
RegisterNetEvent('mrp_phone:client:serviceDispatch', function(data)
    ...
    exports['mrp_fonts']:SetBlipName(blip, data.title or 'Skubus iškvietimas')
...
RegisterNetEvent('mrp_phone:client:hospitalWake', function(data)
    local c = data
