...
AddEventHandler('mrp_phone:local:AfterHospitalWake', function()
    wasDown = false
    downSinceMs = nil
    holdGMs = 0.0
    lastNuiUpdateMs = 0
...
local function tryRequestMedic()
    ...
    TriggerServerEvent('mrp_phone:server:medicRequestFromDead')
end
    ...
    while true do
        ...
        if down then
            if not wasDown then
                ...
                lastNuiUpdateMs = 0
                startDeathFx()
                TriggerServerEvent('mrp_phone:server:reportDeath')
            end
        else
            if wasDown then
                wasDown = false
                ...
                lastNuiUpdateMs = 0
                stopDeathFx()
                TriggerServerEvent('mrp_phone:server:reportAlive')
            end
        end
        Wait(100)
    end
            ...
            if elapsed >= needSec * 1000 then
                if IsControlPressed(0, 47) or IsDisabledControlPressed(0, 47) then
                    ...
                    if holdGMs >= needHold then
                        holdGMs = 0.0
                        TriggerServerEvent('mrp_phone:server:hospitalWake')
                        Wait(900)
                    end
                else
                    holdGMs = 0.0
