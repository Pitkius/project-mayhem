...
local function trySprayGraffiti()
    ...
    GangRunProgressAsync('gang_graffiti', 'Žymi teritoriją...', cfg.durationMs or 4500, {
        ...
        ClearPedTasks(ped)
        TriggerServerEvent('mrp_gangs:server:placeGraffiti', turfId)
    end, function()
        ClearPedTasks(ped)
    end)
end

RegisterNetEvent('mrp_gangs:client:useSprayCan', function()
    trySprayGraffiti()
end)

RegisterNetEvent('mrp_gangs:client:turfInfluenceUpdated', function()
    TriggerServerEvent('mrp_gangs:server:requestTabletRefresh')
end)
