...
local function openAdminMenu()
    ...
    QBCore.Functions.TriggerCallback('mrp_gangs:server:getAdminSnapshot', function(res)
...
RegisterNetEvent('mrp_gangs:client:refreshTablet', function()
    QBCore.Functions.TriggerCallback('mrp_gangs:server:getTabletState', function(res)
        if not res or not res.ok then return end
...
local function tryGangSellToNpc(entity)
    ...
    if GetResourceState('mrp_drugs') == 'started' and exports['mrp_drugs']:IsDrugSellAnimBusy() then
        return
    end
    ...
    local function doSell()
        QBCore.Functions.TriggerCallback('mrp_gangs:server:tryDrugSale', function(res)
            if not res or not res.ok then
            ...
            if res.alertPolice then
                ...
                TriggerServerEvent('mrp_dispatch:server:createServiceCall', 'police', 'drug', 'Pranešta apie įtartiną sandorį', { x = cp.x, y = cp.y, z = cp.z })
            end
        end, turfId, NetworkGetNetworkIdFromEntity(entity))
    end

    if GetResourceState('mrp_drugs') == 'started' then
        exports['mrp_drugs']:PlayDrugSellAnim(entity, function(ok)
...
RegisterCommand('gangtablet', function()
    TriggerEvent('mrp_gangs:client:openTablet')
end, false)
...
RegisterNUICallback('gangs:adminIssueWarning', function(data, cb)
    ...
    TriggerServerEvent('mrp_gangs:server:adminIssueWarning', data.gangId, tostring(data.reason or ''))
...
RegisterNUICallback('gangs:adminClearWarnings', function(data, cb)
    ...
    TriggerServerEvent('mrp_gangs:server:adminClearWarnings', data.gangId)
    SetTimeout(200, function()
        refreshAdminSnapshot(cb)
    end)
end)
...
RegisterNUICallback('gangs:adminDeleteGang', function(data, cb)
    ...
    TriggerServerEvent('mrp_gangs:server:adminDeleteGang', data.gangId)
    SetTimeout(150, function()
        refreshAdminSnapshot(cb)
    end)
end)
...
RegisterNUICallback('gangs:adminSaveTurf', function(data, cb)
    ...
    TriggerServerEvent('mrp_gangs:server:adminSetTurfOwner', data.turfId, tonumber(data.ownerGangId) or 0)
    TriggerServerEvent('mrp_gangs:server:adminSetTurfProgress', data.turfId, tonumber(data.progress) or 0)
    SetTimeout(150, function()
        refreshAdminSnapshot(cb)
    end)
end)
...
RegisterNUICallback('gangs:adminResetTurf', function(data, cb)
    ...
    TriggerServerEvent('mrp_gangs:server:adminResetTurf', data.turfId)
    SetTimeout(150, function()
        refreshAdminSnapshot(cb)
    end)
end)
...
RegisterNUICallback('gangs:createGang', function(data, cb)
    ...
    TriggerServerEvent('mrp_gangs:server:createGang', {
        name = data.name,
...
RegisterNUICallback('gangs:inviteMember', function(data, cb)
    ...
    TriggerServerEvent('mrp_gangs:server:inviteMember', targetId)
    cb({ ok = true })
end)
...
RegisterNUICallback('gangs:setMemberRank', function(data, cb)
    ...
    TriggerServerEvent('mrp_gangs:server:setMemberRank', citizenid, rank)
    cb({ ok = true })
end)
...
RegisterNUICallback('gangs:startMission', function(data, cb)
    ...
    local turfId = tostring(data and data.turfId or '')
    TriggerEvent('mrp_gangs:client:startMission', turfId, missionType)
    closeTabletUi()
    cb({ ok = true })
end)
...
RegisterNUICallback('gangs:kickMember', function(data, cb)
    ...
    TriggerServerEvent('mrp_gangs:server:kickMember', citizenid)
    cb({ ok = true })
end)
...
CreateThread(function()
    ...
    if v.blip and v.blip.enabled then
        ...
        exports['mrp_fonts']:SetBlipName(b, v.blip.label or 'Gang Tablet NPC')
    end
    ...
    if GetResourceState('qb-target') == 'started' then
        ...
        CreateThread(function()
            while vendorPed and DoesEntityExist(vendorPed) do
                local waitMs = 400
                local ped = PlayerPedId()
