...
RegisterNetEvent('fivempro_ikaitas:client:beAggressor', function(targetSid)
    if role then return end
    if not loadDict(Config.AnimDict) then
        notify('Nepavyko užkrauti animacijos.', 'error')
        TriggerServerEvent('fivempro_ikaitas:server:abort')
        return
    end
    ...
    CreateThread(function()
        while role == 'aggressor' and peerSid do
            ...
            if IsDisabledControlJustPressed(0, Config.KeyRelease) then
                TriggerServerEvent('fivempro_ikaitas:server:release')
                break
            end
            if IsDisabledControlJustPressed(0, Config.KeyKill) then
                TriggerServerEvent('fivempro_ikaitas:server:kill')
                break
            end
...
RegisterNetEvent('fivempro_ikaitas:client:beVictim', function(aggressorSid)
    if role then return end
...
RegisterNetEvent('fivempro_ikaitas:client:executeVictim', function()
    local ped = PlayerPedId()
    DetachEntity(ped, true, false)
    ClearPedSecondaryTask(ped)
...
RegisterNetEvent('fivempro_ikaitas:client:stop', function(reason)
...
RegisterCommand('ikaitas', function()
    ...
    TriggerServerEvent('fivempro_ikaitas:server:tryStart', target)
end, false)
