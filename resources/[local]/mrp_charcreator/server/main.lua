...
local function giveStarterItems(src)
    ...
    for _, v in pairs(QBCore.Shared.StarterItems) do
        ...
        exports['qb-inventory']:AddItem(src, v.item, v.amount, false, info, 'mrp_charcreator:starter')
    end
end
...
local function finishLoad(src)
    ...
    if GetResourceState('mrp_spawnfix') == 'started' then
        exports['mrp_spawnfix']:SyncQBCoreAdmin(src)
    else
...
RegisterNetEvent('mrp_charcreator:server:sessionStart', function()
    local src = source
    ...
    if #rows > 0 and Config.AutoLoginExistingCharacter ~= false then
        ...
        if QBCore.Player.Login(src, row.citizenid) then
            ...
            if Player then
                ...
                if GetResourceState('mrp_spawnfix') == 'started' then
                    exports['mrp_spawnfix']:SanitizeLoginState(Player)
                end
            end
            finishLoad(src)
            return
    ...
    TriggerClientEvent('mrp_charcreator:client:openWizard', src)
end)

QBCore.Functions.CreateCallback('mrp_charcreator:server:getSession', function(src, cb)
    local license = QBCore.Functions.GetIdentifier(src, 'license')
...
RegisterNetEvent('mrp_charcreator:server:saveAppearance', function(payload)
    local src = source
    ...
    TriggerClientEvent('mrp_charcreator:client:applyAppearance', src, {
        model = payload.model,
        skin = payload.skin,
    })
...
RegisterNetEvent('mrp_charcreator:server:selectCharacter', function(citizenid)
    local src = source
    citizenid = tostring(citizenid or '')
    if citizenid == '' then return end
    if QBCore.Player.Login(src, citizenid) then
        local Player = QBCore.Functions.GetPlayer(src)
        if Player and GetResourceState('mrp_spawnfix') == 'started' then
            exports['mrp_spawnfix']:SanitizeLoginState(Player)
        end
        finishLoad(src)
    else
...
RegisterNetEvent('mrp_charcreator:server:createCharacter', function(payload)
    local src = source
    if type(payload) ~= 'table' then return end
    ...
    TriggerClientEvent('mrp_charcreator:client:finishCreate', src, {
        model = payload.model,
        skin = payload.skin,
        spawn = spawn,
    })
...
RegisterNetEvent('mrp_charcreator:server:deleteCharacter', function(citizenid)
    local src = source
    ...
    TriggerClientEvent('mrp_charcreator:client:refreshList', src)
end)

RegisterNetEvent('mrp_charcreator:server:savePreset', function(name, skin)
    local src = source
    local license = QBCore.Functions.GetIdentifier(src, 'license')
...
QBCore.Functions.CreateCallback('mrp_charcreator:server:getPresets', function(src, cb)
    local license = QBCore.Functions.GetIdentifier(src, 'license')
...
QBCore.Functions.CreateCallback('mrp_charcreator:server:saveClothingShop', function(src, cb, model, skinJson, originalSkinJson)
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then
...
QBCore.Functions.CreateCallback('mrp_charcreator:server:saveTattooShop', function(src, cb, model, skinJson, originalSkinJson)
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then
        return cb(false, 'Klaida.')
    end
