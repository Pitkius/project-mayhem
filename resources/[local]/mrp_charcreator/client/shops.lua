...
RegisterNetEvent('mrp_charcreator:client:openShop', function(arg1, _)
    local kind = arg1
...
RegisterNetEvent('mrp_charcreator:client:openClothingShop', function()
    ShopSession.Open('clothing')
end)

RegisterNetEvent('mrp_charcreator:client:openBarberShop', function()
    ShopSession.Open('barber')
end)

RegisterNetEvent('mrp_charcreator:client:openTattooShop', function()
...
RegisterNUICallback('saveShop', function(_, cb)
    ...
    if ShopSession.kind == 'clothing' then
        QBCore.Functions.TriggerCallback('mrp_charcreator:server:saveClothingShop', function(ok, msg)
            if ok then
                ShopSession.originalSkinJson = nil
    ...
    if ShopSession.kind == 'tattoo' then
        QBCore.Functions.TriggerCallback('mrp_charcreator:server:saveTattooShop', function(ok, msg)
            if ok then
                ShopSession.originalSkinJson = nil
                ShopSession.Teardown(false)
                QBCore.Functions.Notify(msg or 'Išsaugota.', 'success')
