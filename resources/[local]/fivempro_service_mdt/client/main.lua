...
local function openMdt(service)
    ...
    QBCore.Functions.TriggerCallback('fivempro_service_mdt:server:canOpen', function(can)
        if not can then
            return QBCore.Functions.Notify('Neturite prieigos prie šio MDT.', 'error')
        end
        QBCore.Functions.TriggerCallback('fivempro_service_mdt:server:mdtContext', function(ctx)
...
RegisterNetEvent('fivempro_service_mdt:client:open', function(service)
    openMdt(service)
end)

RegisterNetEvent('fivempro_service_mdt:client:forceClose', function()
    closeMdt()
end)
...
RegisterNUICallback('issueInvoice', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('fivempro_service_mdt:server:issueInvoice', function(result)
...
RegisterNUICallback('searchInvoices', function(data, cb)
    ...
    data = data or {}
    data.service = activeService
    QBCore.Functions.TriggerCallback('fivempro_service_mdt:server:searchInvoices', function(result)
...
RegisterNUICallback('recentInvoices', function(_, cb)
    if not activeService then return cb({ ok = false, rows = {} }) end
    QBCore.Functions.TriggerCallback('fivempro_service_mdt:server:recentInvoices', function(result)
        cb(result or { ok = false, rows = {} })
    end, activeService)
end)
