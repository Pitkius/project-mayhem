...
QBCore.Functions.CreateCallback('fivempro_service_mdt:server:canOpen', function(src, cb, service)
...
QBCore.Functions.CreateCallback('fivempro_service_mdt:server:mdtContext', function(src, cb, service)
    service = tostring(service or '')
    local cfg = serviceCfg(service)
...
QBCore.Functions.CreateCallback('fivempro_service_mdt:server:issueInvoice', function(src, cb, data)
...
QBCore.Functions.CreateCallback('fivempro_service_mdt:server:searchInvoices', function(src, cb, data)
    local service = tostring(data and data.service or '')
...
QBCore.Functions.CreateCallback('fivempro_service_mdt:server:recentInvoices', function(src, cb, service)
    service = tostring(service or '')
    if not serviceCfg(service) then return cb({ ok = false, rows = {} }) end
    local P = QBCore.Functions.GetPlayer(src)
    if not P or not jobMatchesService(P.PlayerData.job.name, service) then
