...
local function unlockVehicleDoors(veh)
    ...
    if GetResourceState('fivempro_basics') == 'started' then
        pcall(function()
            exports['fivempro_basics']:MarkNpcVehicleUnlocked(veh)
        end)
    end

    local netId = NetworkGetNetworkIdFromEntity(veh)
    if netId and netId > 0 then
        TriggerServerEvent('fivempro_hud:server:setVehicleLock', netId, false)
    end
end
...
local function runLockpickMinigame(advanced, onDone)
    ...
    QBCore.Functions.Progressbar('fivempro_vehicle_lockpick', advanced and 'Laužiate spyną (pažangus)…' or 'Laužiate spyną…', advanced and 9000 or 12000, false, true, {
...
RegisterNetEvent('fivempro_basics:client:vehicleLockpickResult', function(data)
    data = data or {}
...
RegisterNetEvent('lockpicks:UseLockpick', function(advanced)
    ...
    runLockpickMinigame(advanced == true, function(completed)
        ...
        TriggerServerEvent('fivempro_basics:server:vehicleLockpick', netId, plate, vehicleLabel(veh), advanced == true)
    end)
end)
