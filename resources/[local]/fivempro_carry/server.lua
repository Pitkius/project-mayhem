...
local function beginCarry(carrier, target, variant)
    ...
    TriggerClientEvent('fivempro_carry:client:start', carrier, {
        role = 'carrier',
        peer = target,
        variant = variant,
    })
    TriggerClientEvent('fivempro_carry:client:start', target, {
        role = 'carried',
        peer = carrier,
        variant = variant,
    })
...
RegisterNetEvent('fivempro_carry:server:request', function(payload)
    local src = source
...
RegisterNetEvent('fivempro_carry:server:breakPair', function()
    local src = source
    local meta = PairMeta[src]
    if not meta then return end
    local peer = meta.peer
    clearPair(src, peer)
    TriggerClientEvent('fivempro_carry:client:stop', src)
    TriggerClientEvent('fivempro_carry:client:stop', peer)
end)

RegisterNetEvent('fivempro_carry:server:stop', function()
    local src = source
    local meta = PairMeta[src]
    if not meta then
