...
local function syncUnitBlips()
    ...
    for _, u in ipairs(serviceState.units or {}) do
        if u.source ~= myId then
            ...
            exports['mrp_fonts']:SetBlipName(b, ('%s %s'):format(u.callsign ~= '' and ('[' .. u.callsign .. ']') or '', u.name or 'Pareigūnas'))
        end
    end
...
RegisterNetEvent('mrp_dispatch:client:update', function(payload)
    ...
    for _, c in ipairs(payload.calls or {}) do
        if not oldIds[c.id] then
            ...
            SetBlipScale(b, 1.0)
            SetBlipAsShortRange(b, false)
            exports['mrp_fonts']:SetBlipName(b, c.callTypeLabel or 'Dispatch')
...
RegisterNetEvent('mrp_dispatch:client:panic', function(data)
    ...
    exports['mrp_fonts']:SetBlipName(b, 'PANIC ALERT')
...
RegisterNetEvent('mrp_dispatch:client:panicClear', function(data)
...
RegisterCommand('panic', function()
    ...
    TriggerServerEvent('mrp_dispatch:server:panic', { x = c.x, y = c.y, z = c.z })
end, false)
...
RegisterCommand('servicemdt', function()
    ...
    QBCore.Functions.TriggerCallback('mrp_dispatch:server:getSnapshot', function(res)
                        ...
                        local callMenu = {
                            { header = ('Iškvietimas %s'):format(c.id), isMenuHeader = true },
                            {
                                ...
                                params = {
                                    isAction = true,
                                    event = function()
                                        TriggerServerEvent('mrp_dispatch:server:updateCallStatus', c.id, 'accept')
                            ...
                            },
                            {
                                ...
                                params = {
                                    isAction = true,
                                    event = function()
                                        TriggerServerEvent('mrp_dispatch:server:updateCallStatus', c.id, 'enroute')
                            ...
                            },
                            {
                                ...
                                params = {
                                    isAction = true,
                                    event = function()
                                        TriggerServerEvent('mrp_dispatch:server:updateCallStatus', c.id, 'done')
                            ...
                            },
                            {
                                ...
                                params = {
                                    isAction = true,
                                    event = function()
                                        TriggerServerEvent('mrp_dispatch:server:updateCallStatus', c.id, 'reject')
        ...
        menu[#menu + 1] = {
            ...
            params = {
                isAction = true,
                event = function()
                    TriggerServerEvent('mrp_dispatch:server:createCrew', '')
...
RegisterCommand('servicecall', function(_, args)
    ...
    local c = GetEntityCoords(PlayerPedId())
    TriggerServerEvent(
        'mrp_dispatch:server:createServiceCall',
        service,
        callType,
...
RegisterCommand('callsign', function(_, args)
    ...
    TriggerServerEvent('mrp_dispatch:server:setCallsign', cs)
end, false)

--- Kliento GPS → state bag (tiksliau nei serverio GetEntityCoords kai kuriems hostams)
local lastDispatchGps = nil
local lastDispatchGpsPushAt = 0
CreateThread(function()
    while true do
        ...
        if s then
            ...
            local moved = not lastDispatchGps or #(c - lastDispatchGps) > 2.5
            local now = GetGameTimer()
            if moved or (now - lastDispatchGpsPushAt) >= 500 then
                ...
                lastDispatchGps = c
                lastDispatchGpsPushAt = now
            end
            Wait(moved and 180 or 380)
        else
            lastDispatchGps = nil
            lastDispatchGpsPushAt = 0
