...
if type(Config) ~= 'table' then
    print('^1[mrp_dispatch] Config is nil — config.lua failed to load^0')
    Config = {}
end
...
local function pushServiceUpdate(service)
    ...
    for _, src in ipairs(QBCore.Functions.GetPlayers() or {}) do
        if isServiceMember(src, service) then
            TriggerClientEvent('mrp_dispatch:client:update', src, payload)
        end
    end
end
...
QBCore.Functions.CreateCallback('mrp_dispatch:server:getMdtSnapshot', function(src, cb, service)
...
RegisterNetEvent('mrp_dispatch:server:createCrew', function(callsign)
    local src = source
...
RegisterNetEvent('mrp_dispatch:server:joinCrew', function(crewId)
    local src = source
    local service = playerService(src)
...
RegisterNetEvent('mrp_dispatch:server:addToCrew', function(crewId, targetSrc)
    local src = source
    local service = playerService(src)
    local crew = crewId and Crews[crewId]
...
RegisterNetEvent('mrp_dispatch:server:deleteCrew', function(crewId)
    local src = source
    local service = playerService(src)
    local crew = crewId and Crews[crewId]
    if not service or not crew or crew.service ~= service then return end
...
RegisterNetEvent('mrp_dispatch:server:leaveCrew', function()
    local src = source
    local crewId = PlayerCrew[src]
...
RegisterNetEvent('mrp_dispatch:server:setCallsign', function(callsign)
    local src = source
    local service = playerService(src)
    if not service then return end
    ...
    elseif action == 'panic_off' and c.panic then
        ...
        for _, id in ipairs(QBCore.Functions.GetPlayers() or {}) do
            if isServiceMember(id, 'police') then
                TriggerClientEvent('mrp_dispatch:client:panicClear', id, { callId = c.id })
            end
        end
    end
    local crewId = PlayerCrew[src]
...
local function triggerOfficerPanic(src, clientPos)
    ...
    for _, id in ipairs(QBCore.Functions.GetPlayers() or {}) do
        if isServiceMember(id, 'police') then
            TriggerClientEvent('mrp_dispatch:client:panic', id, {
                callId = c.id,
...
RegisterNetEvent('mrp_dispatch:server:panic', function(clientPos)
    local src = source
