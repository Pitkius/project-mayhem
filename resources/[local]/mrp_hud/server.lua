...
RegisterNetEvent('mrp_hud:server:setVehicleLock', function(netId, locked)
    local src = source
    if not playerNearVehicle(src, netId, 12.0) then return end
    TriggerClientEvent('mrp_hud:client:syncVehicleLock', -1, netId, locked == true)
end)

--- Durų būsena (atidaryta/uždaryta) – sinchronizacija visiems žaidėjams.
RegisterNetEvent('mrp_hud:server:setVehicleDoor', function(netId, doorIndex, open)
    local src = source
    ...
    TriggerClientEvent('mrp_hud:client:syncVehicleDoor', -1, netId, doorIndex, open == true)
end)
