...
local function openReports(view)
    ...
    QBCore.Functions.TriggerCallback('fivempro_reports:server:getBootstrap', function(data)
...
RegisterNetEvent('fivempro_reports:client:open', function(view)
    openReports(view)
end)

RegisterNetEvent('fivempro_reports:client:staffAlert', function()
    PlaySoundFrontend(-1, 'Event_Start_Text', 'GTAO_FM_Events_Soundset', true)
end)

RegisterNetEvent('fivempro_reports:client:refreshPlayer', function()
    if not isOpen then return end
    QBCore.Functions.TriggerCallback('fivempro_reports:server:myReports', function(rows)
        SendNUIMessage({ action = 'myReports', rows = rows or {} })
    end)
end)

RegisterNetEvent('fivempro_reports:client:refreshAdmin', function()
    if not isOpen then return end
    QBCore.Functions.TriggerCallback('fivempro_reports:server:adminReports', function(rows)
...
RegisterNUICallback('submit', function(payload, cb)
    QBCore.Functions.TriggerCallback('fivempro_reports:server:submit', function(result)
...
RegisterNUICallback('setStatus', function(data, cb)
    QBCore.Functions.TriggerCallback('fivempro_reports:server:setStatus', function(result)
        if result and result.ok then
...
RegisterNUICallback('reply', function(data, cb)
    QBCore.Functions.TriggerCallback('fivempro_reports:server:reply', function(result)
        if result and result.ok then
