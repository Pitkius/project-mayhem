...
local KVP_SETTINGS = 'mrp_radio:settings'
local KVP_ALIAS = 'mrp_radio:alias'
...
RegisterNetEvent('mrp_radio:client:open', function()
    openRadio()
end)

RegisterNetEvent('mrp_radio:client:notify', function(msg, ntype)
    notify(msg, ntype)
...
RegisterNetEvent('mrp_radio:client:freqDenied', function(msg)
    sendNui('freqResult', { ok = false, message = msg })
    notify(msg or 'Neturite prieigos.', 'error')
end)

RegisterNetEvent('mrp_radio:client:freqOk', function(freq, label, lock, alias)
...
RegisterNetEvent('mrp_radio:client:setChannel', function(freq, label, lock, connected)
...
RegisterNetEvent('mrp_radio:client:channelUpdate', function(freq, list)
    if currentFreq and RadioFreq.toKey(freq) == RadioFreq.toKey(currentFreq) then
        members = list or {}
    end
end)

RegisterNetEvent('mrp_radio:client:voiceJoin', function(freq)
...
RegisterNetEvent('mrp_radio:client:voiceLeave', function()
    voiceLeave()
...
RegisterNUICallback('connect', function(data, cb)
    ...
    radioAlias = alias
    saveAlias()
    TriggerServerEvent('mrp_radio:server:connect', freq, radioAlias)
    cb('ok')
end)

RegisterNUICallback('disconnect', function(_, cb)
    TriggerServerEvent('mrp_radio:server:disconnect')
    currentFreq = nil
    currentLabel = nil
    currentLock = nil
    voiceConnected = false
...
RegisterNUICallback('validateFreq', function(data, cb)
    ...
    radioAlias = alias
    saveAlias()
    TriggerServerEvent('mrp_radio:server:validateFrequency', freq, radioAlias)
    cb('ok')
end)
