...
local function removeFromAllChannels(src)
    for key, members in pairs(ChannelMembers) do
        if members[src] then
            members[src] = nil
            TriggerClientEvent('mrp_radio:client:channelUpdate', -1, RadioFreq.fromKey(key), buildMemberList(key))
        end
    end
end
...
local function broadcastChannel(freqKey)
    ...
    for src, _ in pairs(ChannelMembers[freqKey] or {}) do
        TriggerClientEvent('mrp_radio:client:channelUpdate', src, freq, payload)
    end
end
...
local function setPlayerChannel(src, freq)
    ...
    if not freq then
        TriggerClientEvent('mrp_radio:client:setChannel', src, nil, nil, nil, false)
        return
    end
    ...
    TriggerClientEvent('mrp_radio:client:setChannel', src, freq, label, lock, true)
    broadcastChannel(key)
end

RegisterNetEvent('mrp_radio:server:connect', function(rawFreq, alias)
    local src = source
    alias = tostring(alias or ''):gsub('^%s+', ''):gsub('%s+$', ''):sub(1, 32)
    if alias == '' then
        return TriggerClientEvent('mrp_radio:client:notify', src, 'Įrašyk savo vardą racijoje.', 'error')
    end
    local ok, result = canAccessFrequency(src, rawFreq)
    if not ok then
        return TriggerClientEvent('mrp_radio:client:notify', src, result or 'Negalima prisijungti.', 'error')
    end
    local freq = result
    RadioAlias[src] = alias
    setPlayerChannel(src, freq)
    TriggerClientEvent('mrp_radio:client:voiceJoin', src, freq)
    TriggerClientEvent('mrp_radio:client:notify', src, ('Prisijungta: %s MHz · %s'):format(
        RadioFreq.format(freq), GetChannelMeta(freq)
    ), 'success')
end)

RegisterNetEvent('mrp_radio:server:disconnect', function()
    local src = source
    removeFromAllChannels(src)
    TriggerClientEvent('mrp_radio:client:setChannel', src, nil, nil, nil, false)
    TriggerClientEvent('mrp_radio:client:voiceLeave', src)
end)

RegisterNetEvent('mrp_radio:server:validateFrequency', function(rawFreq, alias)
    local src = source
    alias = tostring(alias or ''):gsub('^%s+', ''):gsub('%s+$', ''):sub(1, 32)
    if alias ~= '' then
        RadioAlias[src] = alias
    ...
    local ok, result = canAccessFrequency(src, rawFreq)
    if not ok then
        return TriggerClientEvent('mrp_radio:client:freqDenied', src, result)
    end
    local freq = result
    TriggerClientEvent('mrp_radio:client:freqOk', src, freq, GetChannelMeta(freq), GetRestrictedLabel(freq), alias)
end)
...
QBCore.Functions.CreateUseableItem('radio', function(source)
    TriggerClientEvent('mrp_radio:client:open', source)
end)
