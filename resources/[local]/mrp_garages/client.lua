...
local function openGarageUi(garage)
    QBCore.Functions.TriggerCallback('mrp_garages:server:getPlayerVehicles', function(vehicles)
        garagePreviewMods = {}
        garagePreviewFuel = {}
        for _, v in ipairs(vehicles or {}) do
...
local function doGarageVehicleSpawn(data)
    ...
    QBCore.Functions.TriggerCallback('mrp_garages:server:spawnVehicle', function(result)
        ...
        elseif GetResourceState('mrp_fuel') == 'started' then
            pcall(function()
                exports['mrp_fuel']:SetFuel(veh, GetVehicleFuelLevel(veh))
            end)
        end
...
RegisterNetEvent('mrp_garages:client:forceCloseUi', function()
    closeGarageUi()
end)

RegisterNetEvent('mrp_garages:client:openGarage', function(data)
...
RegisterNetEvent('mrp_garages:client:spawnVehicle', function(data)
    doGarageVehicleSpawn(data)
end)

RegisterNetEvent('mrp_garages:client:parkVehicle', function(data)
    local garageCfg
...
local function createGarageMapBlips()
    ...
    if Config.UseSingleGarageMapBlip then
        ...
        exports['mrp_fonts']:SetBlipName(blip, Config.GarageMapBlipLabel or 'Garažai')
    ...
    for _, garage in ipairs(Config.Garages) do
        if canShowGarageBlip(garage) then
            ...
            SetBlipAsShortRange(blip, true)
            exports['mrp_fonts']:SetBlipName(blip, garage.label)
            local cat = Config.GarageMapBlipCategory
            if cat and cat > 0 then
                SetBlipCategory(blip, cat)
            end
                    ...
                    if spawn and desk then
                        ...
                        if not uiOpen and not IsNuiFocused() then
                                ...
                                if veh ~= 0 and GetPedInVehicleSeat(veh, -1) == ped then
                                    ...
                                    if kmh <= maxSpd then
                                        ...
                                        if IsControlJustPressed(0, 38) and (GetGameTimer() - lastGarageInteractMs) > 450 then
                                            ...
                                            TriggerEvent('mrp_garages:client:parkVehicle', { garageId = garage.id })
                                        end
                                    end
                                end
                            elseif not IsPedInAnyVehicle(ped, false) and dDesk < openR then
                                ...
                                if IsControlJustPressed(0, 38) and (GetGameTimer() - lastGarageInteractMs) > 450 then
                                    lastGarageInteractMs = GetGameTimer()
                                    TriggerEvent('mrp_garages:client:openGarage', { garageId = garage.id })
                                end
                            end
                        end
                    end
...
CreateThread(function()
    ...
    for _, garage in ipairs(Config.Garages) do
        if not garage.policeOnly and not garage.mechanicOnly and not garage.emsOnly and not garage.taxiOnly and not garage.rangerOnly then
            exports['qb-target']:AddBoxZone(('mrp_garage_%s'):format(garage.id), garage.coords, 2.4, 2.4, {
                name = ('mrp_garage_%s'):format(garage.id),
                ...
                options = {
                    {
                        type = 'client',
                        event = 'mrp_garages:client:openGarage',
                    ...
                    {
                        type = 'client',
                        event = 'mrp_garages:client:parkVehicle',
