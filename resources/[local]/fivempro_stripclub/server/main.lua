...
QBCore.Functions.CreateCallback('fivempro_stripclub:server:tryPay', function(src, cb, payType, seatId)
    local price = 0
    if payType == 'lap' then
...
RegisterNetEvent('fivempro_stripclub:server:releaseSeat', function(seatId)
    local key = seatKey(seatId)
