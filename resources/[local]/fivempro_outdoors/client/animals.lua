...
local function tryGutCorpse(entity, meat, extra)
    ...
    QBCore.Functions.TriggerCallback('fivempro_outdoors:server:hasLicense', function(has)
        ...
        Wait(500)
        local ok = exports['fivempro_outdoors']:OpenMinigame('butcher', { label = 'Skerdimo minigame — rodyklės' })
        if ok then
            guttedCorpses[entity] = true
            TriggerServerEvent('fivempro_outdoors:server:gutAnimal', meat, extra)
