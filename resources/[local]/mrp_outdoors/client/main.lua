...
--- Licencijos rodymas
RegisterNetEvent('mrp_outdoors:client:showLicense', function(info, title)
    info = info or {}
...
RegisterNetEvent('mrp_outdoors:client:startLicenseTest', function(data)
    local testType = data.testType
    QBCore.Functions.TriggerCallback('mrp_outdoors:server:canTakeLicenseTest', function(ok, msg)
        ...
        TriggerEvent('mrp_outdoors:client:showLicenseQuestion')
    end, testType)
end)

RegisterNetEvent('mrp_outdoors:client:showLicenseQuestion', function()
    ...
    for ai, ans in ipairs(q.answers) do
        menu[#menu + 1] = {
            header = ans,
            params = {
                event = 'mrp_outdoors:client:licensePick',
                args = { chosen = ai },
            },
        }
    end
...
RegisterNetEvent('mrp_outdoors:client:licensePick', function(data)
    if not testState then return end
    ...
    if testState.index > #testState.questions then
        TriggerServerEvent('mrp_outdoors:server:submitLicenseTest', testState.testType, testState.score, #testState.questions)
        testState = nil
        return
    end
    TriggerEvent('mrp_outdoors:client:showLicenseQuestion')
...
RegisterNetEvent('mrp_outdoors:client:tryFish', function()
    ...
    QBCore.Functions.TriggerCallback('mrp_outdoors:server:hasLicense', function(has)
        ...
        ClearPedTasks(ped)
        if ok then
            TriggerServerEvent('mrp_outdoors:server:fishReward')
        else
    ...
    for raw, clean in pairs(Config.ButcherMap) do
        ...
        if rawItem then
            menu[#menu + 1] = {
                ...
                params = {
                    isAction = true,
                    event = 'mrp_outdoors:client:butcherStart',
                    args = { item = raw },
                },
            }
        end
...
RegisterNetEvent('mrp_outdoors:client:butcherStart', function(data)
    local ok = OpenMinigame('butcher', { label = 'Spausk rodykles pagal instrukcijas' })
    if ok then
        TriggerServerEvent('mrp_outdoors:server:butcherItem', data.item)
    else
    ...
    for item, price in pairs(Config.SellPrices) do
        ...
        if it then
            menu[#menu + 1] = {
                ...
                params = {
                    isAction = true,
                    event = 'mrp_outdoors:client:sellOne',
                    args = { item = item },
                },
            }
        end
...
RegisterNetEvent('mrp_outdoors:client:sellOne', function(data)
    TriggerServerEvent('mrp_outdoors:server:sellItem', data.item, 1)
end)

--- Gamtos parduotuvė (qb-inventory UI)
local function openFishingSupplyShop()
    TriggerServerEvent('mrp_outdoors:server:openFishingShop')
end

local function openHuntingSupplyShop()
    TriggerServerEvent('mrp_outdoors:server:openHuntingShop')
end
...
CreateThread(function()
    ...
    exports['mrp_fonts']:SetBlipName(blip, b.label)
    ...
    if shopLoc and shopLoc.blip then
        ...
        exports['mrp_fonts']:SetBlipName(shopBlip, sb.label)
    end
    ...
    for _, z in ipairs(Config.FishingZones) do
        ...
        exports['mrp_fonts']:SetBlipName(bl, z.label or fb.label)
    end
    ...
    for _, z in ipairs(Config.HuntingZones) do
        ...
        SetBlipAsShortRange(bl, true)
        exports['mrp_fonts']:SetBlipName(bl, z.label or hb.label)
    end
        ...
        options = {
            {
                ...
                action = function() TriggerEvent('mrp_outdoors:client:startLicenseTest', { testType = 'fishing' }) end,
            },
            {
                icon = 'fas fa-id-card',
                label = 'Medžioklės licencijos testas ($' .. Config.LicenseTestPrice .. ')',
                action = function() TriggerEvent('mrp_outdoors:client:startLicenseTest', { testType = 'hunting' }) end,
            },
        },
        distance = 2.0,
    })
...
--- Musket ragdoll
RegisterNetEvent('mrp_outdoors:client:musketRagdoll', function()
    local ped = PlayerPedId()
