...
QBCore.Functions.CreateCallback('mrp_outdoors:server:hasLicense', function(source, cb, licenseItem)
    local Player = QBCore.Functions.GetPlayer(source)
    cb(hasLicense(Player, licenseItem))
end)

QBCore.Functions.CreateCallback('mrp_outdoors:server:canTakeLicenseTest', function(source, cb, testType)
    local Player = QBCore.Functions.GetPlayer(source)
...
RegisterNetEvent('mrp_outdoors:server:openFishingShop', function()
    openLicensedShop(source, Config.FishingShop)
end)

RegisterNetEvent('mrp_outdoors:server:openHuntingShop', function()
    openLicensedShop(source, Config.HuntingShop)
end)

RegisterNetEvent('mrp_outdoors:server:submitLicenseTest', function(testType, score, total)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
RegisterNetEvent('mrp_outdoors:server:sellItem', function(itemName, amount)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
AddEventHandler('weaponDamageEvent', function(sender, data)
    ...
    if victimSrc and victimSrc > 0 then
        TriggerClientEvent('mrp_outdoors:client:musketRagdoll', victimSrc)
    end
end)
...
QBCore.Functions.CreateUseableItem('fishingrod', function(source)
    TriggerClientEvent('mrp_outdoors:client:tryFish', source)
end)
