...
RegisterNUICallback('bankGetState', function(_, cb)
    bankCb('mrp_phone:server:bankGetState', {}, cb)
end)

RegisterNUICallback('bankLookupRecipient', function(data, cb)
    bankCb('mrp_phone:server:bankLookupRecipient', data, cb)
end)

RegisterNUICallback('bankTransfer', function(data, cb)
    bankCb('mrp_phone:server:bankTransfer', data, cb)
end)

RegisterNUICallback('bankDeposit', function(data, cb)
    bankCb('mrp_phone:server:bankDeposit', data, cb)
end)

RegisterNUICallback('bankWithdraw', function(data, cb)
    bankCb('mrp_phone:server:bankWithdraw', data, cb)
end)

RegisterNUICallback('bankGetHistory', function(data, cb)
    bankCb('mrp_phone:server:bankGetHistory', data, cb)
end)
...
CreateThread(function()
    for _, coords in ipairs(Config.BankLocations) do
        ...
        exports['mrp_fonts']:SetBlipName(blip, 'Bankas')
    end
end)

CreateThread(function()
    for i, coords in ipairs(Config.BankLocations) do
        exports['qb-target']:AddCircleZone(('mrp_bank_%s'):format(i), coords, 1.2, {
            name = ('mrp_bank_%s'):format(i),
            debugPoly = false,
            useZ = true
        }, {
            options = {
