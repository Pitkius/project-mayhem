...
local function createBlip()
    ...
    exports['mrp_fonts']:SetBlipName(debugBlip, bl.label or 'TEST: Heist įrankiai')
end
...
local function spawnDebugVendor()
    ...
    exports['qb-target']:AddTargetEntity(debugPed, {
        options = {
            {
                type = 'client',
                event = 'mrp_hacking:client:debugOpenHeistShop',
                icon = 'fas fa-mask',
                        ...
                        if it then
                            rows[#rows + 1] = {
                                ...
                                params = {
                                    isAction = true,
                                    event = function()
                                        TriggerServerEvent('mrp_hacking:server:debugBuyHeistItem', e.item)
                                    end,
                                },
                            }
                        end
...
RegisterNetEvent('mrp_hacking:client:debugOpenHeistShop', function()
    TriggerServerEvent('mrp_hacking:server:debugOpenHeistShop')
end)
