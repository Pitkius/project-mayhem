...
RegisterNetEvent('mrp_interrogation:client:cancelPickSuspect', function()
    pendingPick = nil
end)

RegisterNetEvent('mrp_interrogation:client:confirmPickSuspect', function(args)
    ...
    TriggerServerEvent(
        'mrp_interrogation:server:requestStart',
...
RegisterNetEvent('mrp_interrogation:client:startPickSuspect', function(data)
    ...
    for i, p in ipairs(nearby) do
        menu[#menu + 1] = {
            ...
            params = {
                event = 'mrp_interrogation:client:confirmPickSuspect',
                args = { index = i },
            },
        }
    end
    menu[#menu + 1] = {
        header = 'Atšaukti',
        params = { event = 'mrp_interrogation:client:cancelPickSuspect' },
    }
    exports['qb-menu']:openMenu(menu)
end)

RegisterNetEvent('mrp_interrogation:client:consentPrompt', function(data)
    exports['qb-menu']:openMenu({
        ...
        { header = 'Sutinku (be žalos RP)', params = { event = 'mrp_interrogation:client:consentReply', args = { accept = true } } },
        { header = 'Atsisakau', params = { event = 'mrp_interrogation:client:consentReply', args = { accept = false } } },
    })
end)

RegisterNetEvent('mrp_interrogation:client:consentReply', function(data)
    TriggerServerEvent('mrp_interrogation:server:consent', data and data.accept == true)
end)

RegisterNetEvent('mrp_interrogation:client:sessionStarted', function(data)
    activeSession = data
...
RegisterNetEvent('mrp_interrogation:client:sessionEnded', function()
    activeSession = nil
...
RegisterNetEvent('mrp_interrogation:client:syncState', function(state)
...
RegisterNetEvent('mrp_interrogation:client:toggleDoor', function(groupId)
...
RegisterNUICallback('interrUi', function(data, cb)
    ...
    elseif act == 'slap' or act == 'spotlight' or act == 'start_intense' or act == 'seat' then
        TriggerServerEvent('mrp_interrogation:server:action', act)
    ...
    elseif act == 'finish_police' then
        ...
        TriggerServerEvent('mrp_interrogation:server:endSession', {
            result = 'cooperative',
    ...
    elseif act == 'finish_gang' then
        SetNuiFocus(false, false)
        SendNUIMessage({ action = 'gangControls', show = false })
        TriggerServerEvent('mrp_interrogation:server:endSession', { result = 'complied' })
    elseif act == 'cancel' then
        SetNuiFocus(false, false)
        TriggerServerEvent('mrp_interrogation:server:cancelSession')
    end
    cb('ok')
end)
