...
function Kits.syncAll(target)
    TriggerClientEvent('mrp_interrogation:client:syncKits', target or -1, Kits.list())
end
...
RegisterNetEvent('mrp_interrogation:server:placeKit', function(x, y, z, heading)
    local src = source
...
RegisterNetEvent('mrp_interrogation:server:pickupKit', function(kitId)
    local src = source
...
CreateThread(function()
    ...
    QBCore.Functions.CreateUseableItem(item, function(source)
        TriggerClientEvent('mrp_interrogation:client:startPlaceKit', source)
    end)
end)
