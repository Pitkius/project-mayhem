...
function adminLog(src, action, message, meta)
    if GetResourceState('mrp_playerlog') ~= 'started' then return end
    pcall(function()
        exports['mrp_playerlog']:LogPlayer(src, 'interrogation', action, message, meta, 'orange')
    end)
end
...
local function syncState(key)
    ...
    s.pendingNoise = nil
    broadcastSession(key, 'mrp_interrogation:client:syncState', payload)
end
...
local function endSessionInternal(key, reason)
    ...
    if not s then return end
    Sessions[key] = nil
    broadcastSession(key, 'mrp_interrogation:client:sessionEnded')
...
RegisterNetEvent('mrp_interrogation:server:requestStart', function(locationKind, locationId, suspectSrc)
    local src = source
    ...
    TriggerClientEvent('mrp_interrogation:client:consentPrompt', suspectSrc, {
...
RegisterNetEvent('mrp_interrogation:server:consent', function(accept)
    local src = source
    ...
    TriggerClientEvent('mrp_interrogation:client:sessionStarted', pending.leadSrc, startPayload)
    TriggerClientEvent('mrp_interrogation:client:sessionStarted', src, {
        role = 'suspect',
        key = pending.key,
        locationKind = pending.locationKind,
        locationId = pending.locationId,
...
RegisterNetEvent('mrp_interrogation:server:action', function(action)
    local src = source
    ...
    for key, s in pairs(Sessions) do
        ...
        if s.mode == 'police' then
            ...
            elseif action == 'start_intense' then
                ...
                TriggerClientEvent('mrp_interrogation:client:showPoliceControls', src, true)
            ...
            elseif action == 'door' then
                ...
                if st and st.doorGroupId then
                    TriggerClientEvent('mrp_interrogation:client:toggleDoor', src, st.doorGroupId)
                end
            end
        else
            if action == 'seat' then
...
RegisterNetEvent('mrp_interrogation:server:endSession', function(payload)
    local src = source
    ...
    for key, s in pairs(Sessions) do
        ...
        if s.mode == 'police' then
            ...
            local saved = false
            pcall(function()
                saved = exports['mrp_ltpd']:SaveInterrogationRecord(src, record) == true
            end)
            if saved then
...
RegisterNetEvent('mrp_interrogation:server:cancelSession', function()
    local src = source
    for key, s in pairs(Sessions) do
...
RegisterNetEvent('mrp_interrogation:server:buyTestKit', function()
