...
RegisterNetEvent('fivempro_ambulance:client:bossOpenMenu', function()
    ...
    local menu = {
        { header = 'EMS vadovybė', isMenuHeader = true },
        {
            header = 'Įdarbinti',
            txt = 'Serverio ID + rangas',
            params = { event = 'fivempro_ambulance:client:bossHireInput' },
        },
        {
            header = 'Atleisti',
            txt = 'Serverio ID',
            params = { event = 'fivempro_ambulance:client:bossFireInput' },
        },
        {
            header = 'Keisti rangą',
            txt = 'Serverio ID + naujas rangas',
            params = { event = 'fivempro_ambulance:client:bossGradeInput' },
        },
        {
            header = 'Tarnyba',
            txt = 'Įjungti / išjungti savo duty',
            params = { event = 'fivempro_ambulance:client:bossToggleDuty' },
        },
    }
    TriggerEvent('qb-menu:client:openMenu', menu, false, true)
end)

RegisterNetEvent('fivempro_ambulance:client:bossHireInput', function()
    if not canOpenBoss() then return end
    if GetResourceState('qb-input') ~= 'started' then return end
    local r = exports['qb-input']:ShowInput({
        header = 'Įdarbinti EMS',
        submitText = 'Toliau',
        inputs = {
    ...
    TriggerServerEvent('fivempro_ambulance:server:bossHire', tonumber(r.pid), tonumber(r.grade))
end)

RegisterNetEvent('fivempro_ambulance:client:bossFireInput', function()
    if not canOpenBoss() then return end
    if GetResourceState('qb-input') ~= 'started' then return end
    local r = exports['qb-input']:ShowInput({
        header = 'Atleisti EMS',
    ...
    if not r or not r.pid then return end
    TriggerServerEvent('fivempro_ambulance:server:bossFire', tonumber(r.pid))
end)

RegisterNetEvent('fivempro_ambulance:client:bossGradeInput', function()
    if not canOpenBoss() then return end
    if GetResourceState('qb-input') ~= 'started' then return end
    local r = exports['qb-input']:ShowInput({
        header = 'Keisti rangą',
    ...
    if not r or not r.pid then return end
    TriggerServerEvent('fivempro_ambulance:server:bossSetGrade', tonumber(r.pid), tonumber(r.grade))
end)

RegisterNetEvent('fivempro_ambulance:client:bossToggleDuty', function()
