...
RegisterNetEvent('fivempro_ambulance:client:useElevator', function(data)
...
CreateThread(function()
    ...
    for _, elevator in ipairs(Config.PillboxElevators or {}) do
        for pi, panel in ipairs(elevator.panels or {}) do
            local zoneId = ('fivempro_pillbox_lift_%s_%s'):format(elevator.id, pi)
            exports['qb-target']:AddBoxZone(zoneId, panel.coords, panel.length or 0.5, panel.width or 0.5, {
                ...
                options = {
                    {
                        type = 'client',
                        event = 'fivempro_ambulance:client:useElevator',
