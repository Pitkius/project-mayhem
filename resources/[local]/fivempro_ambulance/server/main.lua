...
RegisterNetEvent('fivempro_ambulance:server:bossHire', function(targetId, grade)
    local src = source
...
RegisterNetEvent('fivempro_ambulance:server:bossFire', function(targetId)
    local src = source
    if not canBoss(src) or not nearManagement(src) then
        return TriggerClientEvent('QBCore:Notify', src, 'Negalima.', 'error')
    end
...
RegisterNetEvent('fivempro_ambulance:server:bossSetGrade', function(targetId, grade)
    local src = source
    if not canBoss(src) or not nearManagement(src) then
        return TriggerClientEvent('QBCore:Notify', src, 'Negalima.', 'error')
    end
