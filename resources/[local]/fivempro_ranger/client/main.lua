...
RegisterNetEvent('fivempro_ranger:client:toggleDuty', function()
    local P = QBCore.Functions.GetPlayerData()
    if not P or not P.job or P.job.name ~= Config.JobName then return end
    TriggerServerEvent('QBCore:ToggleDuty')
end)

RegisterNetEvent('fivempro_ranger:client:openGarage', function(data)
...
RegisterNetEvent('fivempro_ranger:client:openLocker', function()
    local P = QBCore.Functions.GetPlayerData()
...
RegisterNetEvent('fivempro_ranger:client:openPersonalStash', function()
    if not isRangerOnDuty() then
        return QBCore.Functions.Notify('Tik tarnyboje.', 'error')
    end
    TriggerServerEvent('fivempro_ranger:server:openStash')
end)

RegisterNetEvent('fivempro_ranger:client:cuffedState', function(state)
    ...
    for _, p in ipairs(Config.FinePresets or {}) do
        menu[#menu + 1] = {
            ...
            params = {
                isAction = true,
                event = function()
                    TriggerServerEvent('fivempro_ranger:server:issueFine', targetId, p.code, p.label, p.defaultAmount, '')
                end,
            },
        }
    end
    menu[#menu + 1] = {
        ...
        params = {
            ...
            event = function()
                ...
                TriggerServerEvent('fivempro_ranger:server:issueFine', targetId, 'CUSTOM', input.reason or 'Pažeidimas', tonumber(input.amount) or 0, input.reason or '')
            end,
        },
    }
    TriggerEvent('qb-menu:client:openMenu', menu, false, true)
        ...
        options = {
            {
                ...
                action = function(entity)
                    ...
                    TriggerServerEvent('fivempro_ranger:server:cuffPlayer', GetPlayerServerId(idx))
                end,
            },
            {
