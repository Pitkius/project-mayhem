...
function StopLtpdBodycamView()
    destroyBodycamView()
    TriggerEvent('mrp_ltpd:client:mdtCctvFocus', true)
end
...
RegisterNetEvent('mrp_ltpd:client:bodycamUseItem', function()
...
RegisterNetEvent('mrp_ltpd:client:surveillanceStopAll', function()
    destroyBodycamView()
end)

RegisterNUICallback('bodycamList', function(_, cb)
    QBCore.Functions.TriggerCallback('mrp_ltpd:server:bodycamList', function(res)
    ...
    while true do
        if bodycamEquipped then
            if IsEntityDead(PlayerPedId()) then
                TriggerServerEvent('mrp_ltpd:server:bodycamForceOff', 'death')
