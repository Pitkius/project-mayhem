...
RegisterNetEvent('mrp_npcshops:server:openFoodShop', function()
    local src = source
...
RegisterNetEvent('mrp_npcshops:server:openPharmacyShop', function()
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
    if not player then return end
    registerShop(Config.PharmacyShop)
...
RegisterNetEvent('mrp_npcshops:server:openJunkShop', function()
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
    if not player then return end
    registerShop(Config.JunkShop)
