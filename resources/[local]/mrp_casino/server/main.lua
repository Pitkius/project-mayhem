...
local function rollJackpotCar(reason)
    ...
    saveJackpotState()
    TriggerClientEvent('mrp_casino:client:jackpotCarUpdated', -1, jackpotCar)
end
...
-- Callbacks
QBCore.Functions.CreateCallback('mrp_casino:server:getJackpotCar', function(_, cb)
    cb(jackpotCar or {})
end)
...
QBCore.Functions.CreateCallback('mrp_casino:server:blackjackAction', function(src, cb, action)
...
QBCore.Functions.CreateCallback('mrp_casino:server:playSlots', function(src, cb, bet)
    validateCasinoPlay(src, function(ok, Player, stats)
        if not ok then return cb({ ok = false, msg = Player }) end
        local amount, err = clampBet(bet)
        if not amount then return cb({ ok = false, msg = err }) end
...
RegisterNetEvent('mrp_casino:server:rollDice', function(count, sides)
    local src = source
    ...
    for _, playerId in ipairs(GetPlayers()) do
        ...
        if tped and tped ~= 0 then
            ...
            if #(coords - tc) <= radius then
                TriggerClientEvent('mrp_casino:client:showDiceRoll', tid, src, rolls, sides, total)
            end
        end
    end
end)
