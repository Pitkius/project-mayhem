...
local function openActionMenu()
    local menu = {
        ...
        },
        {
            header = 'Hit',
            txt = 'Imti kortą',
            params = { event = 'mrp_casino:client:bjAction', args = { action = 'hit' } },
        },
        {
            header = 'Stand',
            txt = 'Sustoti',
            params = { event = 'mrp_casino:client:bjAction', args = { action = 'stand' } },
        },
        {
            header = 'Double',
            txt = 'Dvigubinti statymą',
            params = { event = 'mrp_casino:client:bjAction', args = { action = 'double' } },
        },
    }
    exports['qb-menu']:openMenu(menu)
end

RegisterNetEvent('mrp_casino:client:bjAction', function(data)
    if not activeSession then return end
    QBCore.Functions.TriggerCallback('mrp_casino:server:blackjackAction', function(res)
...
RegisterNetEvent('mrp_casino:client:openBlackjack', function(_tableId)
    ...
    QBCore.Functions.TriggerCallback('mrp_casino:server:startBlackjack', function(res)
        if not res or not res.ok then
