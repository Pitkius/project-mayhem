...
local function sendMeterState(src, meter)
    TriggerClientEvent('mrp_taxi:client:meterState', src, {
        active = true,
        fare = meter.fare,
...
local function stopMeter(src, notify)
    ...
    TriggerClientEvent('mrp_taxi:client:meterState', src, { active = false })
    if notify then
        if canCharge then
...
RegisterNetEvent('mrp_taxi:server:openStash', function()
    local src = source
...
RegisterNetEvent('mrp_taxi:server:bossHire', function(targetId, grade)
    local src = source
...
RegisterNetEvent('mrp_taxi:server:bossFire', function(targetId)
    local src = source
    if not canBoss(src) or not nearCoords(src, Config.Management.coords, 18.0) then
        return TriggerClientEvent('QBCore:Notify', src, 'Negalima.', 'error')
    end
...
RegisterNetEvent('mrp_taxi:server:bossSetGrade', function(targetId, grade)
    local src = source
    if not canBoss(src) or not nearCoords(src, Config.Management.coords, 18.0) then
        return TriggerClientEvent('QBCore:Notify', src, 'Negalima.', 'error')
    end
...
RegisterNetEvent('mrp_taxi:server:meterStart', function()
    local src = source
    local Player = getTaxiPlayer(src)
    if not Player then
        return TriggerClientEvent('QBCore:Notify', src, 'Tik taksi tarnyboje.', 'error')
...
RegisterNetEvent('mrp_taxi:server:meterStop', function()
