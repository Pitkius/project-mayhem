...
RegisterNetEvent('mrp_taxi:client:toggleDuty', function()
    local P = QBCore.Functions.GetPlayerData()
    if not P or not P.job or P.job.name ~= Config.JobName then return end
    TriggerServerEvent('QBCore:ToggleDuty')
end)

RegisterNetEvent('mrp_taxi:client:openGarageFleet', function()
    if not isTaxiOnDuty() then
        return QBCore.Functions.Notify('Tik taksi tarnyboje.', 'error')
    end
    TriggerEvent('mrp_garages:client:openGarage', { garageId = 'taxi_ls' })
end)

RegisterNetEvent('mrp_taxi:client:openDealershipFleet', function()
    if not isTaxiOnDuty() then
        return QBCore.Functions.Notify('Tik taksi tarnyboje.', 'error')
    end
    TriggerEvent('mrp_dealership:client:openTaxiDealership', 'taxi_ls')
end)

RegisterNetEvent('mrp_taxi:client:openStash', function()
    if not isTaxiOnDuty() then
        return QBCore.Functions.Notify('Tik taksi tarnyboje.', 'error')
    end
    TriggerServerEvent('mrp_taxi:server:openStash')
end)
...
RegisterNetEvent('mrp_taxi:client:openLocker', function()
    if not isTaxiOnDuty() then
    ...
    for idx, outfit in ipairs(Config.DutyOutfits or {}) do
        if grade >= (tonumber(outfit.minGrade) or 0) then
            menu[#menu + 1] = {
                header = outfit.label,
                params = { event = 'mrp_taxi:client:applyOutfit', args = { index = idx } },
            }
        end
    end
    if #menu < 2 then
...
RegisterNetEvent('mrp_taxi:client:applyOutfit', function(data)
...
RegisterNetEvent('mrp_taxi:client:openMeterMenu', function()
    if not isTaxiOnDuty() then
    ...
    local menu = {
        ...
        { header = 'Pradėti važiavimą', txt = 'Sąžininga serverio skaičiuojama kaina', params = { event = 'mrp_taxi:client:meterStart' } },
        { header = 'Užbaigti važiavimą', txt = 'Paskaičiuoti galutinę sumą ir nuskaičiuoti', params = { event = 'mrp_taxi:client:meterStop' } },
    }
    TriggerEvent('qb-menu:client:openMenu', menu, false, true)
end)

RegisterNetEvent('mrp_taxi:client:meterStart', function()
    if not isInAllowedTaxiVehicle() then
        return QBCore.Functions.Notify('Sėsk prie taksi vairo.', 'error')
    end
    TriggerServerEvent('mrp_taxi:server:meterStart')
end)

RegisterNetEvent('mrp_taxi:client:meterStop', function()
    TriggerServerEvent('mrp_taxi:server:meterStop')
end)

RegisterNetEvent('mrp_taxi:client:meterState', function(data)
...
CreateThread(function()
    ...
    exports['mrp_fonts']:SetBlipName(mark, bl.label)
end)
...
CreateThread(function()
    ...
    addMarker({
        ...
        onPress = function()
            ...
            TriggerEvent('qb-menu:client:openMenu', {
                ...
                { header = 'Garažas', params = { event = 'mrp_taxi:client:openGarageFleet' } },
                { header = 'Taksi mašinų pirkimas', params = { event = 'mrp_taxi:client:openDealershipFleet' } },
            }, false, true)
        end,
    })
    ...
    addMarker({
        ...
        canUse = isTaxiOnDuty,
        onPress = function()
            TriggerEvent('mrp_taxi:client:openStash')
        end,
    })
end)
...
CreateThread(function()
    ...
    exports['qb-target']:AddBoxZone('mrp_taxi_locker', lk.coords, 1.65, 1.65, {
        name = 'mrp_taxi_locker',
        heading = lk.heading,
        debugPoly = false,
        ...
        options = {
            {
                type = 'client',
                event = 'mrp_taxi:client:openLocker',
                icon = 'fas fa-shirt',
    ...
    exports['qb-target']:AddBoxZone('mrp_taxi_mgmt', mg, 1.95, 1.95, {
        name = 'mrp_taxi_mgmt',
        ...
        options = {
            {
                type = 'client',
                event = 'mrp_taxi:client:bossOpenMenu',
    ...
    exports['qb-target']:AddBoxZone('mrp_taxi_duty', vector3(Config.Base.x, Config.Base.y, Config.Base.z), 1.85, 1.85, {
        name = 'mrp_taxi_duty',
        ...
        options = {
            {
                type = 'client',
                event = 'mrp_taxi:client:toggleDuty',
