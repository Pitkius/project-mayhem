...
RegisterNetEvent('mrp_taxi:client:bossOpenMenu', function()
    ...
    local menu = {
        { header = 'Taksi vadovybė', isMenuHeader = true },
        {
            ...
            params = { event = 'mrp_taxi:client:bossHireInput' },
        },
        {
            header = 'Atleisti',
            txt = 'Serverio ID',
            params = { event = 'mrp_taxi:client:bossFireInput' },
        },
        {
            header = 'Keisti rangą',
            txt = 'Serverio ID + naujas rangas',
            params = { event = 'mrp_taxi:client:bossGradeInput' },
        },
    }
    TriggerEvent('qb-menu:client:openMenu', menu, false, true)
end)

RegisterNetEvent('mrp_taxi:client:bossHireInput', function()
    ...
    TriggerServerEvent('mrp_taxi:server:bossHire', tonumber(r.pid), tonumber(r.grade))
end)

RegisterNetEvent('mrp_taxi:client:bossFireInput', function()
    if not canOpenBoss() then return end
    if GetResourceState('qb-input') ~= 'started' then return end
    local r = exports['qb-input']:ShowInput({
    ...
    if not r or not r.pid then return end
    TriggerServerEvent('mrp_taxi:server:bossFire', tonumber(r.pid))
end)

RegisterNetEvent('mrp_taxi:client:bossGradeInput', function()
    if not canOpenBoss() then return end
    if GetResourceState('qb-input') ~= 'started' then return end
    local r = exports['qb-input']:ShowInput({
        header = 'Keisti rangą',
    ...
    if not r or not r.pid then return end
    TriggerServerEvent('mrp_taxi:server:bossSetGrade', tonumber(r.pid), tonumber(r.grade))
end)
