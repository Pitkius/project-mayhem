...
RegisterCommand('nesti', function(_, args)
    ...
    TriggerServerEvent('mrp_carry:server:request', { target = closest, variant = variant })
end, false)

RegisterCommand('nenesti', function()
    TriggerServerEvent('mrp_carry:server:stop')
end, false)

RegisterNetEvent('mrp_carry:client:start', function(data)
    ...
    if not peerPed then
        ...
        TriggerServerEvent('mrp_carry:server:breakPair')
        return
    end
...
RegisterNetEvent('mrp_carry:client:stop', function()
    Carry.active = false
    Carry.role = nil
    Carry.peerSid = nil
    Carry.variant = nil
