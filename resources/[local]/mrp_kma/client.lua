...
local function openKmaUi(location)
    ...
    QBCore.Functions.TriggerCallback('mrp_kma:server:getVehicles', function(vehicles)
        kmaPreviewMods = {}
        kmaPreviewFuel = {}
        for _, v in ipairs(vehicles or {}) do
...
RegisterNetEvent('mrp_kma:client:forceCloseUi', function()
    closeKmaUi()
end)

RegisterNetEvent('mrp_kma:client:openUi', function(data)
...
RegisterNUICallback('reclaim', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('mrp_kma:server:reclaim', function(result)
...
CreateThread(function()
    ...
    for _, loc in ipairs((Config.Kma and Config.Kma.locations) or {}) do
        ...
        exports['mrp_fonts']:SetBlipName(blip, loc.label or Config.Kma.blipLabel or 'KMA')
        ...
        exports['qb-target']:AddBoxZone(('mrp_kma_%s'):format(loc.id), loc.coords, size.x, size.y, {
            name = ('mrp_kma_%s'):format(loc.id),
