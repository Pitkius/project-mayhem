...
local function openBossMenu()
    ...
    local menu = {
        { header = 'Gamtos apsaugos vadovybė', isMenuHeader = true },
        {
            ...
            params = { event = 'mrp_ranger:client:bossHireInput' },
        },
        {
            header = 'Atleisti',
            txt = 'Serverio ID',
            params = { event = 'mrp_ranger:client:bossFireInput' },
        },
        {
            header = 'Keisti rangą',
            txt = 'Serverio ID + naujas rangas 0–3',
            params = { event = 'mrp_ranger:client:bossGradeInput' },
        },
        {
            header = 'Tarnyba: įjungti / išjungti',
            txt = 'Tavo duty',
            params = { event = 'mrp_ranger:client:bossToggleDuty' },
        },
    }
    TriggerEvent('qb-menu:client:openMenu', menu, false, true)
end

RegisterNetEvent('mrp_ranger:client:bossOpenMenu', function()
    openBossMenu()
end)

RegisterNetEvent('mrp_ranger:client:bossHireInput', function()
    ...
    TriggerServerEvent('mrp_ranger:server:bossHire', tonumber(r.pid), tonumber(r.grade))
end)

RegisterNetEvent('mrp_ranger:client:bossFireInput', function()
    if not canOpenBoss() then return end
    if GetResourceState('qb-input') ~= 'started' then return end
    local r = exports['qb-input']:ShowInput({
        header = 'Atleisti',
    ...
    if not r or not r.pid then return end
    TriggerServerEvent('mrp_ranger:server:bossFire', tonumber(r.pid))
end)

RegisterNetEvent('mrp_ranger:client:bossGradeInput', function()
    if not canOpenBoss() then return end
    if GetResourceState('qb-input') ~= 'started' then return end
    local r = exports['qb-input']:ShowInput({
        header = 'Keisti rangą',
    ...
    if not r or not r.pid then return end
    TriggerServerEvent('mrp_ranger:server:bossSetGrade', tonumber(r.pid), tonumber(r.grade))
end)

RegisterNetEvent('mrp_ranger:client:bossToggleDuty', function()
    TriggerEvent('mrp_ranger:client:toggleDuty')
end)

RegisterNetEvent('mrp_ranger:client:openDealershipFleet', function(data)
    local P = QBCore.Functions.GetPlayerData()
    if not P or not P.job or P.job.name ~= Config.JobName or not P.job.onduty then
    ...
    TriggerEvent('mrp_dealership:client:openRangerDealership', stationId)
end)
