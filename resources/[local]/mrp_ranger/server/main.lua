...
RegisterNetEvent('mrp_ranger:server:cuffPlayer', function(targetId)
    local src = source
...
RegisterNetEvent('mrp_ranger:server:openStash', function()
    local src = source
...
RegisterNetEvent('mrp_ranger:server:bossHire', function(targetId, grade)
    local src = source
...
RegisterNetEvent('mrp_ranger:server:bossFire', function(targetId)
    local src = source
    if not canBossAction(src) then
        return TriggerClientEvent('QBCore:Notify', src, 'Vadovybė: neturi teisės arba ne tarnyboje.', 'error')
    end
...
RegisterNetEvent('mrp_ranger:server:bossSetGrade', function(targetId, grade)
    local src = source
    if not canBossAction(src) then
        return TriggerClientEvent('QBCore:Notify', src, 'Vadovybė: neturi teisės arba ne tarnyboje.', 'error')
    end
