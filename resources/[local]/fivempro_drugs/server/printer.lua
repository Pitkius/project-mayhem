...
function Printers3d.syncAll(target)
    TriggerClientEvent('fivempro_drugs:client:syncPrinters', target or -1, Printers3d.list())
end
...
RegisterNetEvent('fivempro_drugs:server:pickupPrinter', function(printerId)
    local src = source
...
QBCore.Functions.CreateCallback('fivempro_drugs:server:getPrinterMenu', function(src, cb, printerId)
    local P = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('fivempro_drugs:server:startPrinterCraft', function(printerId, productId)
    local src = source
    ...
    TriggerClientEvent('fivempro_drugs:client:printerCraftStarted', src, printerId, productId, pendingPrint[src].timeMs)
end)

RegisterNetEvent('fivempro_drugs:server:cancelPrinterCraft', function()
    local src = source
...
RegisterNetEvent('fivempro_drugs:server:finishPrinterCraft', function(printerId, productId)
    local src = source
    local pend = pendingPrint[src]
