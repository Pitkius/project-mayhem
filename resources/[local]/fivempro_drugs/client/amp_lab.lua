...
local function startAmpSynthesis()
    ...
    QBCore.Functions.TriggerCallback('fivempro_drugs:server:startAmpSynthesis', function(res)
        ...
        QBCore.Functions.Progressbar('fivempro_amp_synth', 'Amfetamino sintezė…', duration, false, true, {
            ...
            TriggerServerEvent('fivempro_drugs:server:finishAmpSynthesis', token, wrong, vehNetId)
        end, function()
            flareToken = flareToken + 1
            synthesizing = false
            TriggerServerEvent('fivempro_drugs:server:cancelAmpSynthesis', token)
            QBCore.Functions.Notify('Sintezė atšaukta.', 'error')
        end)
    end, vehNetId)
end

RegisterNetEvent('fivempro_drugs:client:ampLabExplode', function(vehNetId)
...
local function setupAmpLab()
    ...
    exports['qb-target']:AddCircleZone('fivempro_amp_lab', lab.coords, 2.2, {
        name = 'fivempro_amp_lab',
        debugPoly = false,
        useZ = true,
    }, {
        options = {
