...
RegisterNetEvent('mrp_mechanic:client:bossOpenMenu', function()
    ...
    local menu = {
        { header = 'Mechanikų vadovybė', isMenuHeader = true },
        {
            ...
            params = { event = 'mrp_mechanic:client:bossHireInput' },
        },
        {
            header = 'Atleisti',
            txt = 'Serverio ID',
            params = { event = 'mrp_mechanic:client:bossFireInput' },
        },
        {
            header = 'Keisti rangą',
            txt = 'Serverio ID + naujas rangas',
            params = { event = 'mrp_mechanic:client:bossGradeInput' },
        },
        {
            header = 'Tarnyba',
            txt = 'Įjungti / išjungti savo duty',
            params = { event = 'mrp_mechanic:client:bossToggleDuty' },
        },
    }
    TriggerEvent('qb-menu:client:openMenu', menu, false, true)
end)

RegisterNetEvent('mrp_mechanic:client:bossHireInput', function()
    ...
    TriggerServerEvent('mrp_mechanic:server:bossHire', tonumber(r.pid), tonumber(r.grade))
end)

RegisterNetEvent('mrp_mechanic:client:bossFireInput', function()
    if not canOpenBoss() then return end
    if GetResourceState('qb-input') ~= 'started' then return end
    local r = exports['qb-input']:ShowInput({
    ...
    if not r or not r.pid then return end
    TriggerServerEvent('mrp_mechanic:server:bossFire', tonumber(r.pid))
end)

RegisterNetEvent('mrp_mechanic:client:bossGradeInput', function()
    if not canOpenBoss() then return end
    if GetResourceState('qb-input') ~= 'started' then return end
    local r = exports['qb-input']:ShowInput({
        header = 'Keisti rangą',
    ...
    if not r or not r.pid then return end
    TriggerServerEvent('mrp_mechanic:server:bossSetGrade', tonumber(r.pid), tonumber(r.grade))
end)

RegisterNetEvent('mrp_mechanic:client:bossToggleDuty', function()
