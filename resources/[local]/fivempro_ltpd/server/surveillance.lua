...
local function hasPerm(src, key)
    return exports['fivempro_ltpd']:HasLtpdPermission(src, key)
end

local function isOnDuty(src)
    return exports['fivempro_ltpd']:IsLtpdOnDuty(src)
end
...
QBCore.Functions.CreateCallback('fivempro_ltpd:server:bodycamList', function(src, cb)
    if not hasPerm(src, 'mdt_bodycam') then return cb({ ok = false }) end
    cb({ ok = true, feeds = buildBodycamList() })
end)

QBCore.Functions.CreateCallback('fivempro_ltpd:server:bodycamWatch', function(src, cb, targetId)
    if not hasPerm(src, 'mdt_bodycam') then return cb({ ok = false }) end
...
RegisterNetEvent('fivempro_ltpd:server:bodycamPanicAutoOn', function()
    local src = source
    ...
    TriggerClientEvent('fivempro_ltpd:client:bodycamState', src, true, 'panic')
end)

QBCore.Functions.CreateUseableItem(Config.Surveillance.BodycamItem or 'police_bodycam', function(source)
    TriggerClientEvent('fivempro_ltpd:client:bodycamUseItem', source)
end)
...
RegisterNetEvent('fivempro_ltpd:server:cctvTamper', function(camId, seconds)
    local src = source
