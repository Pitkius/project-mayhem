    ...
    for _, bike in ipairs(Config.Bikes) do
        menu[#menu + 1] = {
            ...
            params = {
                isAction = true,
                event = function()
                    TriggerServerEvent('fivempro_bikerental:server:rent', locationId, bike.model)
                end,
            },
        }
    end
    if rentedVehicle and DoesEntityExist(rentedVehicle) then
        ...
        menu[#menu + 1] = {
            ...
            params = {
                isAction = true,
                event = function()
                    TriggerServerEvent('fivempro_bikerental:server:returnBike', locationId)
                end,
            },
        }
    end
...
RegisterNetEvent('fivempro_bikerental:client:spawnBike', function(locationId, model, plate)
    local loc
    for _, l in ipairs(Config.Locations) do
        if l.id == locationId then loc = l break end
    end
...
RegisterNetEvent('fivempro_bikerental:client:returnedBike', function(refund)
    deleteRentedBike()
