...
RegisterNetEvent('mrp_ambulance:client:bossOpenMenu', function()
    ...
    local menu = {
        { header = 'EMS vadovybė', isMenuHeader = true },
        {
            ...
            params = { event = 'mrp_ambulance:client:bossHireInput' },
        },
        {
            header = 'Atleisti',
            txt = 'Serverio ID',
            params = { event = 'mrp_ambulance:client:bossFireInput' },
        },
        {
            header = 'Keisti rangą',
            txt = 'Serverio ID + naujas rangas',
            params = { event = 'mrp_ambulance:client:bossGradeInput' },
        },
        {
            header = 'Tarnyba',
            txt = 'Įjungti / išjungti savo duty',
            params = { event = 'mrp_ambulance:client:bossToggleDuty' },
        },
    }
    TriggerEvent('qb-menu:client:openMenu', menu, false, true)
end)

RegisterNetEvent('mrp_ambulance:client:bossHireInput', function()
    ...
    TriggerServerEvent('mrp_ambulance:server:bossHire', tonumber(r.pid), tonumber(r.grade))
end)

RegisterNetEvent('mrp_ambulance:client:bossFireInput', function()
    if not canOpenBoss() then return end
    if GetResourceState('qb-input') ~= 'started' then return end
    local r = exports['qb-input']:ShowInput({
        header = 'Atleisti EMS',
    ...
    if not r or not r.pid then return end
    TriggerServerEvent('mrp_ambulance:server:bossFire', tonumber(r.pid))
end)

RegisterNetEvent('mrp_ambulance:client:bossGradeInput', function()
    if not canOpenBoss() then return end
    if GetResourceState('qb-input') ~= 'started' then return end
    local r = exports['qb-input']:ShowInput({
        header = 'Keisti rangą',
    ...
    if not r or not r.pid then return end
    TriggerServerEvent('mrp_ambulance:server:bossSetGrade', tonumber(r.pid), tonumber(r.grade))
end)

RegisterNetEvent('mrp_ambulance:client:bossToggleDuty', function()
