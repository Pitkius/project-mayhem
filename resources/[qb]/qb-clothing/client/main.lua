...
RegisterNetEvent("qb-clothes:loadSkin", function(_, model, data)
    ...
    if model ~= maleHash and model ~= femaleHash then
        ...
        local fallbackModel = (playerData.charinfo and playerData.charinfo.gender == 1) and 'mp_f_freemode_01' or 'mp_m_freemode_01'
...
RegisterNUICallback('setCurrentPed', function(data, cb)
    local playerData = QBCore.Functions.GetPlayerData()
    local pedIndex = tonumber(data.ped) or 1
    local model = nil

    -- Hard lock to exactly two selectable freemode models.
    if pedIndex == 1 then
        model = 'mp_m_freemode_01'
    elseif pedIndex == 2 then
        model = 'mp_f_freemode_01'
    else
        model = (playerData.charinfo and playerData.charinfo.gender == 1) and 'mp_f_freemode_01' or 'mp_m_freemode_01'
    end
